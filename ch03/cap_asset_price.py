...
# 이게 리스크 수익률 공간상의 효용함수라는데...122쪽 마지막 식...
def U(p):
    mu, sigma = p
    return mu - 1 / 2 * (sigma**2 + mu**2)


# 이게 포트폴리오가 자본시장선에 있을 조건이라고...120쪽 맨 위 식...
...
# 함수를 호출할 sigma 값을 정하고
sigma_ = np.linspace(0.0, 0.5)
# 두 가지 다른 효용 수준에 대해 함수 호출하고
u1_ = f1(sigma_)
u2_ = f2(sigma_)
# 그린다...
plt.figure(figsize=(10, 6))
...
plt.legend()
plt.show()
# sympy는 따로 공부를 좀 해야 할 듯...

# 최적 효용함수에 대한 무차별 곡선 정의
u = sol[0].subs({"b": 1, "v": -opt["fun"]})
print("u", u)
f = lambdify(sigma, u)
# 무차별 곡선을 그리기 위한 수치값 계산...
u_ = f(sigma_)
plt.figure(figsize=(10, 6))
plt.plot(0, r, "o", ms=9, label="risk-less asset")
plt.plot(sigma_M, mu_M, "^", ms=9, label="market portfolio")
...
plt.plot(
    (0, 0.5), (r, r + (mu_M - r) / sigma_M * 0.5), label="capital market line", lw=2.0
)
